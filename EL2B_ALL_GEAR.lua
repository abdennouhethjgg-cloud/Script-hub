--[[
  ...
  Modules:
    • Loading screen (30s + music + skip @20s)
    • Player: Anti Gummy/Ragdoll/Paint/Boogie, Speed 3-mode,
              Tool Aimbot, Drop, Insta V1/V2, TP Down
    • ESP: Player ESP + Tracker + Anti Lag
    • Settings: Mobile buttons, Lock, shape Square, sizes
    • Auto Steal V1/V2/V3, Lagger, Speed Bypass
    • Secure webhook API (no hard-coded secrets)

  UI:
    • Main menu centered
    • 5 action buttons: Square + black contour
    • Close: − | Open: draggable mini
]]
...
pcall(function()
	for _, n in ipairs({
		"EL2BHubFullMenu", "EL2BHubFullMini", "EL2BHubModeBar", "EL2BHubActionButtons",
		"EL2BHubLoading",
	}) do
...
----------------------------------------------------------------
-- EL2B HUB — LOADING SCREEN (30s + music)
----------------------------------------------------------------
do
	local LOAD_DURATION = 30
	local MUSIC_ID = "76650356472656"
	local parentGui = (typeof(gethui) == "function" and gethui()) or game:GetService("CoreGui")
	pcall(function()
		local old = parentGui:FindFirstChild("EL2BHubLoading")
		if old then old:Destroy() end
		old = PlayerGui:FindFirstChild("EL2BHubLoading")
		if old then old:Destroy() end
	end)

	local gui = Instance.new("ScreenGui")
	gui.Name = "EL2BHubLoading"
	...
	glow.BackgroundColor3 = Color3.fromRGB(20, 8, 18)
	glow.BorderSizePixel = 0
	glow.Parent = bg
	local gg = Instance.new("UIGradient")
	gg.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, Color3.fromRGB(12, 8, 16)),
	...
	title.Position = UDim2.fromOffset(10, 22)
	...
	sub.Position = UDim2.fromOffset(10, 58)
	sub.BackgroundTransparency = 1
	sub.Text = "Loading modules..."
	sub.TextColor3 = Color3.fromRGB(200, 160, 190)
	...
	barBg.Position = UDim2.fromOffset(20, 110)
	...
	barFill.BackgroundColor3 = Color3.fromRGB(255, 20, 147)
	...
	pctLbl.Size = UDim2.new(0.5, -10, 0, 22)
	pctLbl.Position = UDim2.fromOffset(20, 136)
	...
	timeLbl.Size = UDim2.new(0.5, -10, 0, 22)
	timeLbl.Position = UDim2.new(0.5, 0, 0, 136)
	timeLbl.BackgroundTransparency = 1
	timeLbl.Text = "30s"
	timeLbl.TextColor3 = Color3.fromRGB(180, 180, 200)
	...
	statusLbl.Position = UDim2.fromOffset(10, 175)
	statusLbl.BackgroundTransparency = 1
	statusLbl.Text = "Initializing..."
	statusLbl.TextColor3 = Color3.fromRGB(160, 140, 170)
	statusLbl.Font = Enum.Font.Gotham
	statusLbl.TextSize = 12
	statusLbl.Parent = card

	-- SKIP button (visible only after 20s)
	local skipBtn = Instance.new("TextButton")
	skipBtn.Name = "SkipBtn"
	skipBtn.Size = UDim2.new(1, -40, 0, 32)
	skipBtn.Position = UDim2.fromOffset(20, 210)
	skipBtn.BackgroundColor3 = Color3.fromRGB(255, 20, 147)
	...
	skipStroke.Color = Color3.fromRGB(0, 0, 0)
	skipStroke.Thickness = 2
	skipStroke.Transparency = 0
	skipStroke.Parent = skipBtn

	-- Label compte à rebours avant affichage du SKIP (20s)
	local skipWaitLbl = Instance.new("TextLabel")
	skipWaitLbl.Name = "SkipWait"
	skipWaitLbl.Size = UDim2.new(1, -40, 0, 18)
	skipWaitLbl.Position = UDim2.fromOffset(20, 214)
	skipWaitLbl.BackgroundTransparency = 1
	skipWaitLbl.Text = "Skip in 20s..."
	skipWaitLbl.TextColor3 = Color3.fromRGB(140, 130, 150)
	skipWaitLbl.Font = Enum.Font.Gotham
	skipWaitLbl.TextSize = 11
	skipWaitLbl.Visible = true
	skipWaitLbl.Parent = card

	local skipped = false
	local function doSkip()
		if skipBtn.Visible and skipBtn.Active then
			skipped = true
		end
	end
	...
	local tips = {
		...
		"Loading Auto Steal...",
		"Loading Speed system...",
		"Loading ESP & Tracker...",
		"Loading mobile buttons...",
		"Loading Lagger modules...",
		"Applying settings...",
		"Almost ready...",
		"Finalizing EL2B Hub...",
	}
	...
	music.Name = "EL2BLoadMusic"
	music.SoundId = "rbxassetid://" .. tostring(MUSIC_ID)
	music.Volume = 0.65
	music.Looped = true
	music.Playing = false
	music.Parent = gui
	...
	task.spawn(function()
		local t0 = tick()
		while gui and gui.Parent and cardStroke and cardStroke.Parent do
			cardGrad.Rotation = ((tick() - t0) * 40) % 360
			task.wait(0.03)
		end
	end)

	-- Progress loop (30 seconds) — Skip visible from 20s
	local SKIP_AT = 20
	local start = tick()
	local tipIdx = 1
	local lastTip = 0
	...
	while true do
		...
		local p = math.clamp(elapsed / LOAD_DURATION, 0, 1)
		barFill.Size = UDim2.new(p, 0, 1, 0)
		pctLbl.Text = string.format("%d%%", math.floor(p * 100 + 0.5))
		local left = math.max(0, math.ceil(LOAD_DURATION - elapsed))
		timeLbl.Text = string.format("%ds", left)
		-- Délai 20s avant affichage du bouton SKIP
		if elapsed < SKIP_AT then
			local remain = math.max(0, math.ceil(SKIP_AT - elapsed))
			skipWaitLbl.Visible = true
			skipWaitLbl.Text = string.format("Skip in %ds...", remain)
			skipBtn.Visible = false
			skipBtn.Active = false
		elseif not skipShown then
			skipShown = true
			skipWaitLbl.Visible = false
			skipBtn.Visible = true
			skipBtn.Active = true
			skipBtn.BackgroundTransparency = 1
			skipBtn.TextTransparency = 1
			pcall(function()
				TS:Create(skipBtn, TweenInfo.new(0.35), {
					BackgroundTransparency = 0,
					TextTransparency = 0,
				}):Play()
			end)
		end
		if tick() - lastTip > (LOAD_DURATION / #tips) then
			statusLbl.Text = tips[tipIdx] or "Loading..."
			tipIdx = math.min(tipIdx + 1, #tips)
			lastTip = tick()
		end
	...
	-- Fade out
	pcall(function()
		TS:Create(music, TweenInfo.new(0.8), { Volume = 0 }):Play()
	end)
	for _, obj in ipairs({ bg, card, title, sub, pctLbl, timeLbl, statusLbl, barBg, barFill, skipBtn, skipWaitLbl }) do
		pcall(function()
			if not obj then return end
			if obj:IsA("TextLabel") then
				TS:Create(obj, TweenInfo.new(0.45), { TextTransparency = 1 }):Play()
			elseif obj:IsA("TextButton") then
				TS:Create(obj, TweenInfo.new(0.45), { TextTransparency = 1, BackgroundTransparency = 1 }):Play()
			elseif obj:IsA("Frame") then
				TS:Create(obj, TweenInfo.new(0.45), { BackgroundTransparency = 1 }):Play()
			end
		end)
	end
	pcall(function()
		TS:Create(cardStroke, TweenInfo.new(0.45), { Transparency = 1 }):Play()
	end)
	task.wait(0.55)
...
function APP.Log(msg)
	print(string.format("[EL2B %s] %s", APP.Version, tostring(msg)))
end
...
function APP.Safe(fn, label)
	...
	if not ok then
		APP.Warn((label or "task") .. ": " .. tostring(err))
	end
	return ok, err
end

-- Cleanup previous instance (hot-reload safe)
APP.Safe(function()
	local names = {
		"EL2BHubFullMenu", "EL2BHubFullMini", "EL2BHubModeBar",
		"EL2BHubActionButtons", "EL2BHubLoading", "VisV2ModeBar",
		"EL2BBypassGui", "EL2BAutoStealGui", "Per1shccLaggerV2",
	}
...
pcall(function()
	if writefile then
		writefile("EL2B_Auto_Steal_Embedded.lua", [==[--[[
  Auto Steal — full EL2B Hub logic V1 / Semi V1-V2 / V3
]]
...
-- Shared state (must match EL2B Hub names used in steal logic)
...
-- ============================================================
task.spawn(function()
	local CONFIG_FILE = "EL2BHubAutoSteal"
	...
	pcall(function()
		local o = parent:FindFirstChild("EL2BAutoStealGui"); if o then o:Destroy() end
		local b = PlayerGui:FindFirstChild("EL2BAutoStealBarGui"); if b then b:Destroy() end
	end)

	local gui = Instance.new("ScreenGui")
	gui.Name = "EL2BAutoStealGui"
...
local St = {
	...
	instaMode = "V1",
	mobileBtns = true, guiLock = false, -- default ON: 5 action buttons
	...
	btnShape = "Square", -- Square + black contour on 5 action buttons
	btnScale = 0.75,
	menuScale = 1.0,
...
function loadCfg()
	local ok, data = pcall(function()
		if isfile and readfile then
			...
			for _, old in ipairs({"RaraOnflop.json", "EL2BHub_FullMenu_v1.json", "EL2BHub_Config.json", "EL2BHub.json"}) do
				if isfile(old) then
	...
	-- Force Square on the 5 action buttons
	if not St.btnShape or St.btnShape == "" then
		St.btnShape = "Square"
	end
...
loadCfg()
-- Guarantee Square default for mobile action buttons
if not St.btnShape then St.btnShape = "Square" end
task.spawn(function()
	while true do
		task.wait(3)
		local ok, err = pcall(saveCfg)
		if not ok then warn("[EL2B] autosave 3s fail:", err) end
	end
end)
...
----------------------------------------------------------------
-- ANTI GUMMY / BOOGIE / PAINTBALL (EL2BHub full) — LUÔN BẬT
--------------------------------------------------------------
...
----------------------------------------------------------------
-- TOOL AIMBOT (EL2BHub full) — LUÔN BẬT
----------------------------------------------------------------
local aimOn = true
...
-- Toujours au centre de l'écran (milieu)
...
menuScaleObj.Name = "EL2BMenuScale"
...
function applyMenuScale()
	...
	if menuScaleObj and menuScaleObj.Parent then
		...
		local baseW, baseH = 310, 380
		Main.Size = UDim2.new(0, math.floor(baseW * sc), 0, math.floor(baseH * sc))
	end
end
pcall(applyMenuScale)

-- Centre la GUI au milieu (ignore ancienne position sauvegardée au premier affichage)
local function centerMainGui()
	Main.AnchorPoint = Vector2.new(0.5, 0.5)
	Main.Position = UDim2.new(0.5, 0, 0.5, 0)
	St._mainPos = {0.5, 0, 0.5, 0}
end
centerMainGui()
_G.EL2B_CenterMenu = centerMainGui
...
toggleNamed(pageSet, "Show Panel TP", St.showTPPanel ~= false, function(on)
	...
	local g = PlayerGui:FindFirstChild("EL2BHubbTP")
...
toggleNamed(pageSet, "Lock UI", St.guiLock, function(on)
	...
	pcall(function()
		...
		local miniGui = PlayerGui:FindFirstChild("EL2BHubFullMini")
		...
		local mb = PlayerGui:FindFirstChild("EL2BHubModeBar")
		...
		snapHoldersLock(PlayerGui:FindFirstChild("EL2BHubActionButtons"))
		snapHoldersLock(PlayerGui:FindFirstChild("EL2BHubModeBar"))
	end)
	pcall(saveCfg)
end, 3, "guiLock")
...
for i, name in ipairs({ "Round", "Box", "Square" }) do
	...
	b.AutoButtonColor = false
	b.Parent = shapeRow
	corner(b, name == "Round" and 14 or (name == "Box" and 6 or 0)) -- Square = 0
...
actionBtn(pageSet, "Save Now", C.accent, function()
	...
	pcall(function()
		...
		local mini = PlayerGui:FindFirstChild("EL2BHubFullMini")
...
Mini.Position = UDim2.new(0.5, 0, 0.5, 0) -- milieu (comme le menu)
...
function openMenu()
	...
	Mini.Visible = false
	St.menuOpen = true
	-- Rester / revenir au milieu de l'écran
	Main.AnchorPoint = Vector2.new(0.5, 0.5)
	if not St._mainPos or (St._mainPos[1] == 0.5 and St._mainPos[2] == 0 and St._mainPos[3] == 0.5 and St._mainPos[4] == 0) then
		Main.Position = UDim2.new(0.5, 0, 0.5, 0)
	end
	pcall(saveCfg)
...
function applyCorner(btn, forceShape)
	...
	-- Remove old corners then recreate (avoid stuck radius)
	for _, ch in ipairs(btn:GetChildren()) do
		if ch:IsA("UICorner") then pcall(function() ch:Destroy() end) end
	end
	local rad
	if shape == "Round" then
		rad = UDim.new(1, 0) -- full circle
	elseif shape == "Box" then
		rad = UDim.new(0, 12) -- soft box
	elseif shape == "Pill" then
		rad = UDim.new(0, 16)
	else
		-- Square: true sharp square (radius 0)
		rad = UDim.new(0, 0)
	end
	local c = Instance.new("UICorner")
	c.Name = "ShapeCorner"
	c.CornerRadius = rad
	c.Parent = btn
	-- Sync background image / dim frames
	for _, ch in ipairs(btn:GetChildren()) do
...
function applyCornerToChildren(btn, shape)
	...
	else rad = UDim.new(0, 0) -- true Square
	end
	for _, ch in ipairs(btn:GetChildren()) do
		if ch:IsA("ImageLabel") or (ch:IsA("Frame") and ch.Name ~= "BtnTextOverlay") then
			for _, sub in ipairs(ch:GetChildren()) do
...
function applyEmpireBtnBg(btn, index)
	...
	-- nền đậm hơn (EL2BHub style) nhưng chữ vẫn đọc được
...
----------------------------------------------------------------
-- SPEED V2 MODE BAR (full GUI from EL2BHub Clean) — 3 mode
----------------------------------------------------------------
local v2BarButtons = {}
...
function makeActBtn(key, label, pos, cb)
	...
	btn.ZIndex = 100
	btn.Parent = holder
	-- 5 action buttons: FORCE Square shape + visible square border
	local forceShape = "Square"
	St.btnShape = St.btnShape or "Square"
	applyCorner(btn, forceShape)
	do
		-- Contour noir distinct (interne)
		local oldS = btn:FindFirstChildOfClass("UIStroke")
		if oldS then oldS:Destroy() end
		local s = Instance.new("UIStroke")
		s.Name = "BtnBorder"
		s.Color = Color3.fromRGB(0, 0, 0) -- noir
		s.Thickness = 3
		s.Transparency = 0
		s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
		s.Parent = btn
	end
	-- Cadre carré externe — contour noir distinct
	do
		local oldF = holder:FindFirstChild("SquareFrame")
		if oldF then oldF:Destroy() end
		local sq = Instance.new("Frame")
		sq.Name = "SquareFrame"
		sq.BackgroundTransparency = 1
		sq.BorderSizePixel = 0
		sq.Size = UDim2.new(1, 4, 1, 4) -- légèrement plus grand pour double trait
		sq.Position = UDim2.new(0, -2, 0, -2)
		sq.ZIndex = (btn.ZIndex or 100) + 2
		sq.Active = false
		sq.Parent = holder
		local sc = Instance.new("UICorner")
		...
		ss.Name = "SquareStroke"
		ss.Color = Color3.fromRGB(0, 0, 0) -- noir distinct
		ss.Thickness = 2.5
		ss.Transparency = 0
	...
	applyCornerToChildren(btn, forceShape)
...
function _G.VisUpdateMobileVisuals()
	...
	for key, e in pairs(actRefs) do
		if e and e.holder and e.btn then
			...
			-- Force Square + contour noir distinct
			applyCorner(e.btn, "Square")
			applyCornerToChildren(e.btn, "Square")
			...
			s.Color = Color3.fromRGB(0, 0, 0) -- contour noir
			s.Thickness = 3
			s.Transparency = 0
			s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
			-- Cadre carré externe noir
			local holder = e.holder
			local sq = holder and holder:FindFirstChild("SquareFrame")
			if not sq and holder then
				sq = Instance.new("Frame")
				sq.Name = "SquareFrame"
				sq.BackgroundTransparency = 1
				sq.BorderSizePixel = 0
				sq.Size = UDim2.new(1, 4, 1, 4)
				sq.Position = UDim2.new(0, -2, 0, -2)
				sq.ZIndex = (e.btn.ZIndex or 100) + 2
				sq.Active = false
				sq.Parent = holder
				local sc = Instance.new("UICorner")
				sc.CornerRadius = UDim.new(0, 0)
			...
			if sq then
				local ss = sq:FindFirstChild("SquareStroke") or sq:FindFirstChildOfClass("UIStroke")
				if not ss then
					ss = Instance.new("UIStroke")
					ss.Name = "SquareStroke"
					ss.Parent = sq
				end
...
if _G._EL2BHubCharReapplyConn then
	pcall(function() _G._EL2BHubCharReapplyConn:Disconnect() end)
	_G._EL2BHubCharReapplyConn = nil
end
_G._EL2BHubCharReapplyConn = LP.CharacterAdded:Connect(function(char)
	task.wait(0.4)
	pcall(function()
...
print("[EL2BHub Full] Player(Anti+Speed S2+Aim+Drop+Insta) | ESP | Settings mobile/lock/shape/size")
...
----------------------------------------------------------------
-- PANEL TP (from EL2BHubb, no music / no image / no sentry)
----------------------------------------------------------------
local TP = {
	base = nil, pet = nil,
...
----------------------------------------------------------------
-- BODY LOCK (full EL2BHub Clean / BloodHounds)
----------------------------------------------------------------
...
task.defer(function()
	...
	if not pagePlayer then warn("[EL2B] pagePlayer nil") return end
...
----------------------------------------------------------------
-- OVERHEAD: real speed + discord (from EL2BHub)
----------------------------------------------------------------
...
print("[EL2B] TP TO BEST (Chilli) ready | key B | nút TP BEST")
...
task.spawn(function()
	local src
	pcall(function()
		if readfile and isfile and isfile("EL2B_Auto_Steal_Embedded.lua") then
			src = readfile("EL2B_Auto_Steal_Embedded.lua")
		end
	end)
	...
	if not src then
		...
		warn("[EL2B] AutoSteal embed not found via readfile — using internal + global hooks")
	else
		local fn, err = loadstring(src)
		if fn then
			pcall(fn)
			print("[EL2B] AutoSteal full loaded")
		else
			warn("[EL2B] AutoSteal load err", err)
		end
	end
...
task.defer(function()
	...
	local mainGui = PlayerGui:FindFirstChild("EL2BHubFullMenu")
...
print("[EL2B Hub] Speed | InfJump Empire | RagdollTP | AutoSteal | Wallpaper | WinLive | Control")
...
task.spawn(function()
	while task.wait(3) do
		pcall(function()
			...
			snapHolders(PlayerGui:FindFirstChild("EL2BHubActionButtons"))
			snapHolders(PlayerGui:FindFirstChild("EL2BHubModeBar"))
			-- Panel TP
			local tp = PlayerGui:FindFirstChild("EL2BHubbTP")
			if tp then
			...
			local asg = PlayerGui:FindFirstChild("EL2BAutoStealGui")
			if asg then
...
task.spawn(function()
	...
	pcall(function()
		local old = PlayerGui:FindFirstChild("EL2BHubbTP")
		if old then old:Destroy() end
	end)
	local gui = Instance.new("ScreenGui")
	gui.Name = "EL2BHubbTP"
	...
	print("[EL2B] Compact TP panel ready | rainbow | markers | key X")
end)
...
----------------------------------------------------------------
-- ===== EL2B Hub FULL: Speed Bypass + Lagger V2 (replace All Gear old panels) =====
-- EL2BBypass — Speed Bypass (Noxa-style UI + keybind PC + minimize + bg)
-- ============================================================
task.spawn(function()
	local Players = game:GetService("Players")
	local LP = Players.LocalPlayer
	...
	local ConfigFile = "EL2BBypass.json"
	...
	_G.AceSetEL2BBypassEnabled = function(on, power)
		if on then startBypass(power or 97000) else stopBypass() end
	end
	_G.AceToggleEL2BBypass = function(power)
		if running then stopBypass() else startBypass(power or 97000) end
		return running
	end
	_G.AceEL2BBypassActive = function() return running end
	...
	pcall(function()
		if isfile and isfile(ConfigFile) then
			...
			if data.Power then _G.AceEL2BBypassPower = tonumber(data.Power) or 97000 end
			...
			_G.AceEL2BBypassPanelOpen = savedPanelOpen
		end
	end)
	...
	local function saveCfg(mainFrame, power, extra)
		local panelIsOpen = (_G.AceEL2BBypassPanelOpen == true)
		pcall(function()
		...
		if mainFrame and mainFrame.Parent then
			...
			savedPos = data.Pos
			_G.AceEL2BBypassPanelPosition = data.Pos
		elseif type(_G.AceEL2BBypassPanelPosition) == "table" then
			data.Pos = _G.AceEL2BBypassPanelPosition
		...
		savedWantedActive = data.WantedActive == true
		_G.AceEL2BBypassPanelOpen = savedPanelOpen
		_G.AceEL2BBypassMinimized = savedMinimized
		_G.AceEL2BBypassWantedActive = savedWantedActive
		_G.AceEL2BBypassPower = data.Power
	...
	pcall(function()
		local old = parent:FindFirstChild("EL2BBypassGui")
		if old then old:Destroy() end
	end)

	local gui = Instance.new("ScreenGui")
	gui.Name = "EL2BBypassGui"
	gui.ResetOnSpawn = false
	gui.IgnoreGuiInset = true
	gui.Enabled = true
	...
	local titleEL2B = Instance.new("TextLabel", header)
	titleEL2B.Size = UDim2.fromOffset(36, 31)
	titleEL2B.Position = UDim2.fromOffset(12, 0)
	titleEL2B.BackgroundTransparency = 1
	titleEL2B.Text = "EL2B"
	titleEL2B.TextColor3 = Color3.fromRGB(255, 255, 255)
	titleEL2B.Font = Enum.Font.GothamBold
	titleEL2B.TextSize = 12
	titleEL2B.TextXAlignment = Enum.TextXAlignment.Left
	titleEL2B.ZIndex = 6
	...
	powerBox.Text = tostring(_G.AceEL2BBypassPower or 97000)
	...
	discLbl.Text = "EL2B Hub · Speed Bypass"
	...
	local function setMinimized(on)
		...
		_G.AceEL2BBypassPanelOpen = true
		pcall(function() saveCfg(main, powerBox.Text) end)
	end
	...
	_G.AceSetEL2BBypassLock = function(on)
		forceSetLocked(on)
	end
		...
		UIS_.InputEnded:Connect(function(input)
			...
			pcall(function()
				...
				if p then
					...
					_G.AceEL2BBypassPanelPosition = savedPos
				end
	...
	local function setPanelVisible(on, opts)
		...
		if not on then
			...
			minimized = false
			uiMinimized = false
			_G.AceEL2BBypassPanelOpen = false
			syncActivateUI()
			saveCfg(main, powerBox.Text, { PanelOpen = false, Minimized = false, WantedActive = false })
			return
		end
		local sp = _G.AceEL2BBypassPanelPosition or savedPos
	...
	_G.AceSetEL2BBypassPanelVisible = setPanelVisible
	_G.AceToggleEL2BBypassPanel = function()
	...
	pcall(function()
		...
		if _G.AceEL2BBypassLocked == true then wantLock = true end
		locked = wantLock
		syncLockUI()
	end)
	task.defer(function()
	...
	print("[EL2B] EL2BBypass panel loaded (Noxa-style + keybind + minimize + remember open)")
end)
...
task.spawn(function()
	...
	local LP = Players.LocalPlayer
	local PlayerGui = LP:WaitForChild("PlayerGui")
	local ConfigFile = "EL2B_PerishLaggerV2.json"
...
print("[EL2B] Perish Lagger V2 FULL GUI loaded (menu toggle, pos persist like V1)")

end)

-- ============================================================
-- WRAPPERS: Settings → EL2B Hub Lagger V2 (Perish) + Speed Bypass
-- ============================================================
do
	...
	function _G.VisSetSpeedBypassPanel(on)
		...
		local function forceGui(on2)
			pcall(function()
				...
				for _, p in ipairs(parents) do
					if p then
						gui = p:FindFirstChild("EL2BBypassGui")
		...
		local api = _G.AceSetEL2BBypassPanelVisible
		if type(api) == "function" then
			local ok, err = pcall(api, on)
			if not ok then warn("[EL2B] SpeedBypass panel:", err) end
		else
			-- API chưa sẵn: đợi tối đa 1s (không 5s)
			task.spawn(function()
				for _ = 1, 10 do
					if type(_G.AceSetEL2BBypassPanelVisible) == "function" then
						pcall(_G.AceSetEL2BBypassPanelVisible, on)
						break
					end
					task.wait(0.1)
				end
...
function _G.VisSetSpeedBypassLock(on)
		...
		pcall(function()
			if _G.AceSetEL2BBypassLock then
				_G.AceSetEL2BBypassLock(on)
			else
				_G.AceEL2BBypassLocked = on
			end
		end)
		pcall(function() if saveCfg then saveCfg() end end)
	end
...
print("[EL2B] FULL EL2B Lagger V2 (Perish) + Speed Bypass GUI (no old Vynx)")
end)
...
-- SÉCURITÉ API :
--   - Aucune clé / URL hard-codée dans le script
--   - URL chargée uniquement depuis _G.EL2B_WinLiveWebhook ou fichier local
--   - Validation stricte du format Discord webhook
--   - Feature désactivée par défaut tant que l'URL n'est pas fournie
-- ============================================================
do
	...
	-- ========= SÉCURISATION ACCÈS API / WEBHOOK =========
	local WEBHOOK_CONFIG_FILE = "EL2B_Webhook.json"
	local WIN_LIVE_HOOK = nil

	local function isValidDiscordWebhook(url)
		if type(url) ~= "string" or #url < 40 then return false end
		-- Format attendu uniquement : https://discord.com/api/webhooks/... ou discordapp.com
		if not (url:find("^https://discord%.com/api/webhooks/%d+/.+")
			or url:find("^https://discordapp%.com/api/webhooks/%d+/.+")
			or url:find("^https://canary%.discord%.com/api/webhooks/%d+/.+")
			or url:find("^https://ptb%.discord%.com/api/webhooks/%d+/.+")) then
			return false
		end
		-- Refuse les placeholders / exemples
		local lower = url:lower()
		if lower:find("your_webhook", 1, true)
			or lower:find("xxx", 1, true)
			or lower:find("token", 1, true)
			or lower:find("example", 1, true)
			or lower:find("replace", 1, true) then
			return false
		end
		return true
	end
	...
	local function loadWebhookUrl()
		-- 1) Priorité : variable globale (définie AVANT le loadstring)
		local fromGlobal = rawget(_G, "EL2B_WinLiveWebhook") or rawget(_G, "AceWinLiveWebhook")
		if isValidDiscordWebhook(fromGlobal) then
			return fromGlobal
		end
		-- 2) Fichier local (jamais commit / jamais partagé)
		local ok, data = pcall(function()
			if isfile and isfile(WEBHOOK_CONFIG_FILE) and readfile then
				return Http:JSONDecode(readfile(WEBHOOK_CONFIG_FILE))
			end
		end)
		if ok and type(data) == "table" and isValidDiscordWebhook(data.url) then
			return data.url
		end
		return nil
	end
	...
	local function saveWebhookUrl(url)
		if not isValidDiscordWebhook(url) then return false end
		pcall(function()
			if writefile then
				writefile(WEBHOOK_CONFIG_FILE, Http:JSONEncode({ url = url, updated = os.time() }))
			end
		end)
		return true
	end

	WIN_LIVE_HOOK = loadWebhookUrl()

	-- API publique sécurisée pour configurer le webhook (sans hard-code)
	_G.EL2B_SetWinLiveWebhook = function(url)
		if not isValidDiscordWebhook(url) then
			warn("[EL2B] Webhook refusé : URL invalide ou non-Discord.")
			return false
		end
		WIN_LIVE_HOOK = url
		_G.EL2B_WinLiveWebhook = url
		saveWebhookUrl(url)
		print("[EL2B] Win Live webhook configuré (stocké localement uniquement).")
		return true
	end

	_G.EL2B_ClearWinLiveWebhook = function()
		WIN_LIVE_HOOK = nil
		_G.EL2B_WinLiveWebhook = nil
		pcall(function()
			if isfile and isfile(WEBHOOK_CONFIG_FILE) and delfile then
				delfile(WEBHOOK_CONFIG_FILE)
			elseif writefile then
				writefile(WEBHOOK_CONFIG_FILE, "{}")
			end
		end)
		print("[EL2B] Win Live webhook effacé.")
	end

	if not WIN_LIVE_HOOK then
		-- Feature silencieuse tant qu'aucune URL valide n'est fournie
		-- Pour activer : _G.EL2B_SetWinLiveWebhook("https://discord.com/api/webhooks/...")
	end
	-- ========= FIN SÉCURISATION =========
	...
	function sendWebhook(winText, data)
		-- Re-valide à chaque envoi (sécurité)
		if not req then return end
		if not WIN_LIVE_HOOK or type(WIN_LIVE_HOOK) ~= "string" then return end
		if not isValidDiscordWebhook(WIN_LIVE_HOOK) then
			WIN_LIVE_HOOK = nil
			return
		end
		...
		local embed = {
			title = "🏆 EL2B Hub — DUEL WON",
			color = color,
			fields = fields,
			footer = { text = "EL2BOnTop EL2BHub | EL2B Hub | " .. os.date("%d/%m/%Y %H:%M") },
	...
	-- Activé seulement si une URL webhook valide a été fournie
	_G.AceWinLiveEnabled = (WIN_LIVE_HOOK ~= nil) and (_G.AceWinLiveEnabled ~= false)
	...
	print("[EL2B Hub] Win Live FULL + image CDN fix (thumbnails.roblox.com → rbxcdn)")
end
...
--
-- Win Live webhook :
--   - Aucune URL / clé dans le code source
--   - Config via : _G.EL2B_SetWinLiveWebhook("https://discord.com/api/webhooks/...")
--   - Ou fichier local EL2B_Webhook.json (jamais partagé)
